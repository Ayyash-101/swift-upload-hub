...
CREATE POLICY "Anyone can read page discussions"
  ON public.page_discussions FOR SELECT USING (true);
CREATE INDEX page_discussions_session_page_idx
  ON public.page_discussions (session_id, page_number, created_at);
...
BEGIN
  UPDATE public.sessions
     SET pdf_url = p_pdf_url, book_name = p_book_name,
         current_page = 1, total_pages = 0, updated_at = now()
   WHERE id = p_session_id AND leader_id = p_user_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Not authorized' USING ERRCODE = '42501'; END IF;
END;
$$;
...
BEGIN
  UPDATE public.participants
     SET current_page = p_current_page, last_seen = now()
...
-- ============ page_discussions RPCs ============

-- Post a message: type is forced to 'leader_note' if caller is the session leader and asks for it,
-- otherwise 'discussion_message'.
CREATE OR REPLACE FUNCTION public.post_page_message(
  p_session_id uuid,
  p_page_number int,
  p_user_id text,
  p_user_name text,
...
) RETURNS public.page_discussions
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_leader_id text;
  v_type public.page_message_type;
  v_row public.page_discussions;
BEGIN
  ...
  END IF;
  IF p_is_leader_note AND v_leader_id = p_user_id THEN
    v_type := 'leader_note';
  ELSE
    v_type := 'discussion_message';
  END IF;
  INSERT INTO public.page_discussions
    (session_id, page_number, participant_id, participant_name, content, type)
...
CREATE OR REPLACE FUNCTION public.update_page_message(
  p_message_id uuid, p_user_id text, p_content text
) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF p_content IS NULL OR length(btrim(p_content)) = 0 THEN
...
-- Delete: own message OR session leader can delete any message in the session.
CREATE OR REPLACE FUNCTION public.delete_page_message(
  p_message_id uuid, p_user_id text
) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_owner text; v_session uuid; v_leader text;
BEGIN
  SELECT participant_id, session_id INTO v_owner, v_session
    FROM public.page_discussions WHERE id = p_message_id;
  ...
  IF p_user_id <> v_owner AND p_user_id <> v_leader THEN
